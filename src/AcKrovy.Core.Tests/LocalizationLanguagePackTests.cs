...
public sealed partial class LocalizationLanguagePackTests
{
    ...
    public void SatelliteResourceFiles_MatchDefaultKeysAndContainNoEmptyValues()
    {
        ...
        Assert.Equal(817, defaultResources.Count);
    ...
    public void DisplayProviders_ReturnConsistentLocalizedElementTypesLengthModesAndSlopeDirections()
    {
        ...
        {
            ["sk-SK"] = (["Krokva", "Pomúrnica", "Väznica", "Stĺpik", "Klieština / hambálok", "Vzpera", "Väzný trám", "Vlastný prvok", "Nárožná krokva", "Úžľabná krokva"], ["Automaticky podľa typu", "Pôdorysná dĺžka", "Prepočítať podľa sklonu", "Ručne zadaná dĺžka"], ["Normálny (začiatok → koniec)", "Obrátený (koniec → začiatok)"]),
            ["cs-CZ"] = (["Krokev", "Pozednice", "Vaznice", "Sloupek", "Kleština / hambálek", "Vzpěra", "Vazný trám", "Vlastní prvek", "Nárožní krokev", "Úžlabní krokev"], ["Automaticky podle typu", "Půdorysná délka", "Přepočítat podle sklonu", "Ručně zadaná délka"], ["Normální (začátek → konec)", "Obrácený (konec → začátek)"]),
            ["en-US"] = (["Rafter", "Wall plate", "Purlin", "Post", "Collar tie", "Brace", "Tie beam", "Custom element", "Hip rafter", "Valley rafter"], ["Automatic by element type", "Plan length", "Correct for slope", "Manually entered length"], ["Normal (start → end)", "Reversed (end → start)"]),
            ["de-DE"] = (["Sparren", "Mauerlatte", "Pfette", "Stütze", "Zange / Kehlbalken", "Strebe", "Bundbalken", "Benutzerdefiniertes Bauteil", "Gratsparren", "Kehlsparren"], ["Automatisch nach Bauteiltyp", "Grundrisslänge", "Nach Neigung umrechnen", "Manuell eingegebene Länge"], ["Normal (Anfang → Ende)", "Umgekehrt (Ende → Anfang)"]),
            ["pl-PL"] = (["Krokiew", "Murłata", "Płatew", "Słup", "Jętka", "Zastrzał", "Belka wiązarowa", "Element niestandardowy", "Krokiew narożna", "Krokiew koszowa"], ["Automatycznie według typu elementu", "Długość w rzucie", "Przelicz według nachylenia", "Długość wprowadzona ręcznie"], ["Normalny (początek → koniec)", "Odwrócony (koniec → początek)"]),
            ["fr-FR"] = (["Chevron", "Sablière", "Panne", "Poteau", "Entrait retroussé", "Contrefiche", "Entrait", "Élément personnalisé", "Chevron d’arêtier", "Chevron de noue"], ["Automatique selon le type d'élément", "Longueur en projection horizontale", "Recalculer selon la pente", "Longueur saisie manuellement"], ["Normal (début → fin)", "Inversé (fin → début)"]),
        };
