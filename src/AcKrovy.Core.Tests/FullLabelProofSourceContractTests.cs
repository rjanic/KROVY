...
public sealed class FullLabelProofSourceContractTests
{
    ...
    public void CommandsPolicyAndService_AreEntirelyDebugOnly()
    {
        ...
        Assert.Contains("AK_DEV_FULLLABEL_TEXT_CREATE", Commands());
        Assert.Contains("AK_DEV_FULLLABEL_TEXT_VERIFY", Commands());
        Assert.Contains("AK_DEV_FULLLABEL_TEXT_CLEAN", Commands());
    }

    [Fact]
    public void Create_UsesRealProductionFullLabelPath()
    {
        ...
        Assert.DoesNotContain("modelSpace.AppendEntity(label)", create);
    ...
    public void Verify_IsReadOnlyAndReportsPresentationFields()
    {
        ...
        Assert.Contains("label.TextStyleId", core);
        Assert.Contains("label.TextHeight", core);
        Assert.Contains("NOT TESTED", Service() + Policy());
        Assert.Contains("DBMOD", verify);
    }

    [Fact]
    public void ProofCoversLegacyExplicitDenominatorPostAndRefresh()
    {
        var policy = Policy();
        Assert.Contains("\"A\"", policy);
        Assert.Contains("IsPostFootprint: true", policy);
        ...
        Assert.Contains(
            ...
            policy);
        Assert.Contains("CreatePostFootprintElementData(", policy);
    }

    private static string Commands() =>
        Source("src", "AcKrovy.AutoCAD", "Commands",
            "AutoCadFullLabelProofCommands.cs");
